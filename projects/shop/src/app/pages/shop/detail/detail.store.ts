...
export const DetailStore = signalStore(
  ...
  withComputed(({ articleId }, shopStore = inject(ShopStore), dealerStore = inject(DealerStore)) => ({
    article: computed(() => (articleId() ? shopStore.priced().find((item) => item.id === articleId()) : null)),
    entityId: computed(() =>
      shopStore
        .entities()
    ...
    dealer: computed(() =>
      dealerStore.entities().find((dealer) => dealer.dealerType === shopStore.entityMap()[articleId()].dealer),
    ),
  })),
  withComputed(({ article }) => ({
    tableData: computed(() => createTableData(article()!)),
  })),
  withMethods((store, shopStore = inject(ShopStore), detailService = inject(DetailService)) => ({
